...
class Randomizer(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        random_list = dict_func()
        self.correcticut = list()
        def key_randomizer():            
                n = len(random_list)
                global r
                r = random.randint(0,n-1)
                
                k = list(random_list.keys())[r]
                if k in self.correcticut:
                    if n == len(self.correcticut):
                        self.correcticut = []
                    key_randomizer()
                else:
                    label_random.config(text = k)
                    self.correcticut.append(k)
        ...
        def show_meaning():
            v = list(random_list.values())[r]
            label_random2.config(text =  v)
